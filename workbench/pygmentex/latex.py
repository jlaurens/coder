...
from pygments.token import Token as PyToken, STANDARD_TYPES
from pygments.util import get_bool_opt, get_int_opt


__all__ = ['CDRLatexFormatter']
...
class CDRLatexFormatter(Formatter):
  r"""
  ...
  The `get_style_defs()` method of a `CDRLatexFormatter` returns a string
...
class LatexEmbeddedLexer(Lexer):
  ...
  def get_tokens_unprocessed(self, text):
    ...
    for i, t, v in self.lang.get_tokens_unprocessed(text):
      if t in PyToken.Comment or t in PyToken.String:
        if buf:
  ...
  def get_tokens_aux(self, index, text):
    while text:
      ...
      if sep1:
        b, sep2, text = text.partition(self.right)
        if sep2:
          yield index + len(sep1), PyToken.Escape, b
          index += len(sep1) + len(b) + len(sep2)
        else:
          yield index, PyToken.Error, sep1
          index += len(sep1)
          text = b

...
\begin{tabular}{p{0.233\columnwidth}p{0.233\columnwidth}p{0.533\columnwidth}}
Text before
\begin{CDRBlock}[
  tags=src,
]
...
\end{tabular}
\end{center}
The line height is inherited from the surrounding environment unless the baselinestrech factor has been provided. Columns 2 and 3 are enclosed in a \pkg{setspace} environment with one and a half line spacing. The last column also adds the \CDRCode[tags={src,options}]|baselinstretch=2| option.
\begin{center}
\setlength{\tabcolsep}{0pt}
...
\CDRCode[tags=src]|\begin{tcolorbox}[#1]| as \MyMeta{begin engine instructions}, where \CDRCode[tags=src]|#1| will be replaced by whatever is provided for key \CDRCode[tags={src,options}]|⟨\MyMeta{engine name}⟩ engine options|, and as \MyMeta{end engine instructions}
...
Notice the \CDRCode[tags=src]|\CDRGetOption| command used to retrieve the value for the key \CDRCode[tags={src,options}]|label|.
%
\begin{center}
\setlength{\arraycolsep}{0pt}
...
In the last column, the option \CDRCode[tags={src,options}]|firstline=L.*2| means: from the first line that matches the regular expression pattern \CDRCode[tags=src]|"L.*2"|, according to \LaTeX3 \url{interface3.pdf}.
Similarly, the option \CDRCode[tags={src,options}]|lastline=L.*4| means: up to the first line, from the line found above, that matches the regular expression pattern \CDRCode[tags=src]|"L.*2"|.
%

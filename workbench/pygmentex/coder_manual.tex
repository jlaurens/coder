...
\begin{description}
\itemtt[\CDRCheckRed show tags=none]: no display
\begin{CDRBlock}[
  tags=none,
  show tags=none,
  numbers=none,
...
no tags displayed
\end{CDRBlock}
\itemtt[\CDRCheckRed show tags=left] display on the left side of the code,
\begin{CDRBlock}[
  tags=left,
  show tags=left,
  numbers=none,
...
a left tag on the left
\end{CDRBlock}
\itemtt[\CDRCheckRed show tags=right] display on the right side of the code,
\begin{CDRBlock}[
  tags=right,
  show tags=right,
  numbers=none,
...
a right tag on the right
\end{CDRBlock}
\itemtt[\CDRCheckRed show tags=numbers] display if numbers are dispayed and on the same side,
\begin{CDRBlock}[
  tags = left,
  show tags=numbers,
  numbers=left,
...
tags and number on the right
\end{CDRBlock}
\itemtt[\CDRCheckRed show tags=mirror] display if numbers are dispayed and on the opposite side,
\end{description}
\end{multicols}
OTHER
\begin{description}
...
Line numbering only makes sense for block code chunks.
Number 
\subsection{Framing}
...
\CDRExport{
  ...
  tags={⟨\MyMeta{tag}\textsubscript{1},\MyMeta{tag}\textsubscript{2},...⟩},
}
\end{CDRBlock}
At the end of the typesetting process, all the code chunks with \MyMeta{tag}\textsubscript{1} like
\begin{CDRBlock}[tags=src]
\begin{⟨CDRBlock⟩}[tags={⟨\MyMeta{some tag},\MyMeta{tag}\textsubscript{1},\MyMeta{other tag},...⟩}]
...
\end{⟨CDRBlock⟩}
\end{CDRBlock}
are collected in the order of the source, then all the code chunks with \MyMeta{tag}\textsubscript{2} are collected in turn,... Finally, the result is written to \MyMeta{output file path}.
...
\itemtt[preamble=\MyMeta{preamble text}]
The \MyMeta{preamble text} is saved before the collected code chunks unless the \CDRCode|raw| option is set to \CDRCode|true|.
Initially empty.
the \MyMeta{postamble text} is saved after the code collected chunks unless the \CDRCode|raw| option is set to \CDRCode|true|. Initially empty.
\itemtt[raw{[=true$\vert$false]}]
When \CDRCode|true|, a preamble and postamble will be added to all the collected code chunks. Initially |false|.
\itemtt[once{[=true$\vert$false]}]
Consider for example the exportation \CDRCode|tags| list
\MyMeta{tag}\textsubscript{1},\MyMeta{tag}\textsubscript{2}.
If a block code chunk has exactly the same tag list, then it should be exported when \MyMeta{tag}\textsubscript{1} codes are collected but also when \MyMeta{tag}\textsubscript{2} codes are collected.
...
Initially |true|: export only once.
\end{description}
%    lang = tex,
%    preamble =,
%    postamble =,

...
\begin{document}
\title{Manual of the \pkg{coder} package}
...
Code chunks are put in a \texttt{CDRBlock} \LaTeX{} environment. Like many verbatim envitonments, the closing command is a full line consisting literally of \CDRCode[lang=tex]|\end{CDRBlock}|, with no extra space.
Optional key--value options are enclosed with square brackets. The opening bracket must follow the \CDRCode[lang=tex]|\begin{CDRBlock}| insruction, on the same line. For the previous example, the input was
\begin{center}
\begin{tabular}{p{0.495\linewidth}|p{0.495\linewidth}}
\begin{minipage}[t]{0.9\linewidth}
...
Inserting code as inline text is possible with the command \cs{CDRCode} which syntax is
\begin{CDRBlock}[tags=src]
\CDRCode[⟨\MyMeta{key--value optional arguments}⟩]?⟨\MyMeta{inline code chunk}⟩?
\end{CDRBlock}
Here the question mark \CDRCode|?| stands for quite any unicode character, one that is not in \MyMeta{inline code chunk} and is not a \CDRCode|[| of course.

\section{Export code with \cs{CDRExport}}
\subsection{Usage}
The syntax for the \cs{CDRExport} command is the following, where both \CDRCode|file| key must be provided with a non empty value,
\begin{CDRBlock}[tags=src]
\CDRExport{
...
If the \CDRCode|tags| list is void, no exportation takes place.
\subsection{Options}
\begin{description}
\itemtt
\end{description}

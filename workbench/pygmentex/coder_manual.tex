...
\CDRCode[tags=src]|\begin{tcolorbox}[#1]| as \MyMeta{begin engine instructions} and
\CDRCode[tags=src]|\end{tcolorbox}| as \MyMeta{end engine instructions}. It can be used to define a custom display engine as well.
%
\begin{center}
\setlength{\arraycolsep}{0pt}
\begin{tabular}{p{0.495\columnwidth}p{0.495\columnwidth}}
The \pkg{coder} package also supports \pkg{fancyvrb} options to display frames, here is an example from the \pkg{fancyvrb} documentation:
\begin{CDRBlock}[
  frame=single,
  framerule=1mm,
  framesep=3mm,

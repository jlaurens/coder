...
\\[0.25\baselineskip]
The colorized code is found in \MyMeta{file name}\verb|-colorized.pdf|%
...
\CDRCode[tags=src]|\CDRCode[⟨\meta{key--value optional arguments}⟩]!\meta{inline code chunk}⟩!|
\vskip0.25\baselineskip

. It is similar to \LaTeX{} \cs{verb} command, except that it accepts optional key--value arguments put into square brackets.

\subsection{Key--value options}

\subsection{About tags}
To each code chunck is associate an ordered list of tags, so far we have seen the tags ``lua'' and ``python'' appearing in the previous examples opposite to the line numbers.

...
This Lua\LaTeX\ package is part of any standard \TeX{} distribution. To use it in a document, put the next instruction in the preamble:
...
\begin{CDRBlock}[tags=src]
\begin{CDRBlockSave}{⟨\MyMeta{identifier}⟩}
...
\end{CDRBlockSave}
\end{CDRBlock}
Next command allows to display that source with any option available
\begin{CDRBlock}[tags=src]
\CDRBlockUse[⟨\MyMeta[1]{key}=\MyMeta[1]{value},\MyMeta[2]{key}=\MyMeta[2]{value},...⟩]{⟨\MyMeta{identifier}⟩}
\end{CDRBlock}
%
...
We can always refer to the page of a specific code extract with the \CDRCode|reflabel=⟨\MyMeta{label name}⟩| option:
\begin{center}
\setlength{\tabcolsep}{0mm}%
\begin{tabular}{p{0.5\linewidth}p{0.5\linewidth}}
\begin{CDRBlock}[
...
\end{tabular}
\end{center}
It can be used when \pkg{pygments} is not available as well.
\begin{center}

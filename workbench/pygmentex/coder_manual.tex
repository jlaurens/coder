...
\CDRSet{
  ...
  tags/options={
    format=\color{CDROption}\bfseries,
  },
}
%%%%%
%%%%%
...
\begin{⟨CDRBlock⟩}[
  ...
  ⟨\textcolor{CDROption}{\textbf{engine options}}⟩={
    colback=magenta!5!white,
    colframe=magenta!80!black,
    boxrule=5\fboxrule,
...
\CDRCode[tags=src]|\begin{tcolorbox}[#1]| as \MyMeta{begin engine instructions}, where \CDRCode[tags=src]|#1| will be replaced by whatever is provided for key \CDRCode[tags=options]|⟨\MyMeta{engine name}⟩ engine options|, and as \MyMeta{end engine instructions}
...
\begin{CDRBlock}[tags=src,no top space]
\begin{⟨CDRBlock⟩}[
  ⟨\color{CDROption}firstline=2⟩,
  ⟨\color{CDROption}lastline=4⟩,
]
Line 1
Line 2 *
Line 3 *
...
\begin{CDRBlock}[tags=src,no top space]
\begin{⟨CDRBlock⟩}[
  ⟨\color{CDROption}firstline=-3⟩,
  ⟨\color{CDROption}lastline=-1⟩,
]
Line 1
Line 2 *
Line 3 *
...
\begin{CDRBlock}[tags=src,no top space]
\begin{⟨CDRBlock⟩}[
  ⟨\color{CDROption}firstline=L.*2⟩,
  ⟨\color{CDROption}lastline=L.*4⟩,
]
Line 1
Line 2 *
Line 3 *
...
The delimiters must not be part of the code, unicode characters $\langle$ and $\rangle$ are suitable amongst many others.

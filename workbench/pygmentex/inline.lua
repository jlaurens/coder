--
-- This is file `inline.lua',
-- generated with the docstrip utility.
--
-- The original source files were:
--
-- inline.dtx  (with options: `luax')
-- 
...
local INLINE_PY_PATH = io.popen([[kpsewhich inline.py]]):read('a'):match("^%s*(.-)%s*$")
...
local function process_inline(self, what)
  ...
  local t = {
    ...
    ['pygment_options']= self.options or {},
  }
  local s = json.tostring(t,true)
  local fh = assert(io.open(self.input_p,'w'))
  fh:write(s, '\n')
  fh:close()
  local cmd = "python3 "..INLINE_PY_PATH..' "'..self.input_p..'"'

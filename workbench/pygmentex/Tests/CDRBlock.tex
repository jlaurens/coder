...
\cs_new:Npn \CDR@Line {
  ...
  \int_compare:nNnTF { \CDR_int_use:c { __step } / 5 * 5 } = { \CDR_int_use:c { __step } } {
    \prg_new_conditional:Nnn \CDR_int_if_display:n { b, T, F, TF } {
      \int_compare:nNnTF { ( #1 ) / \CDR_int_use:c { __step } / \CDR_int_use:c { __step } }
    }
  
  } {
  }
\prg_new_conditional:Nnn \CDR_int_if_zero_mod_v:n { b, T, F, TF } {
}
  \clist_map_inline:nn { firstnumber, stepnumber } {
    \CDR_tag_get:cN {##1} \l_CDR_tl
    \lua_now:n { CDR:hilight_set_var('##1') }
...
% Some setup are made for line numbering.
% \end{function}
%    \begin{MacroCode}
\cs_new:Npn \CDR_line:nnn [ #1 ] {
  \keys_set:nn { CDR@Line } { #1 }
...
% \end{function}
%    \begin{MacroCode}
\cs_set:Npn \CDR_number_display_a:n {
